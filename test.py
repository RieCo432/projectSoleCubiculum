from room import Room
from edge import Edge
from Compass import DataFlow, Direction
import time
from config import build_living_room

living_room = build_living_room()

living_room.hue_off()
time.sleep(2)
living_room.leds.fill((255, 255, 255))
if not living_room.demo:
    living_room.leds.show()
time.sleep(5)
print(living_room.get_brightness())
time.sleep(5)
living_room.set_brightness(final_brightness=0.1)
if not living_room.demo:
    living_room.leds.show()
print(living_room.get_brightness())
time.sleep(5)
living_room.rainbow_ceiling_only(speed=60, include_vertical=True)
time.sleep(5)
living_room.decrease_brightness(percent_per_second=0.1)
living_room.leds_off()
time.sleep(1)
living_room.hue_on()
